...
from ...data import ClearVolume, TissueType, SpinalCenterline
...
def get_centerline(
    ...
    if tissue.tissue_type != TissueType.SPINAL_CORD:
        raise TypeError(
            ...
            f"not {tissue.tissue_type}"
        )

    # 1. We don't care about the densities, only the mask
    volume = np.where(tissue.data > 0, 1, 0)
    ...
    for sl in range(nr_slices):
        ...
        if not np.isnan(center).any():
            ...
            centerline_volume[sl, center[0], center[1]] = 1
...
def fill_nan_coords(centerline_centers: np.ndarray) -> np.ndarray:
    ...
    for col in (1, 2):  # x and z columns
        values = filled[:, col]
