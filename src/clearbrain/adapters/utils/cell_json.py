...
def get_json_cell_data(raw_path: Path, cells_filename: str) -> list:
    ...
    if len(raw_files) == 1:
        ...
        if raw_files[0].stem == cells_filename:
        ...
        # 3.2 if does not load it anyways, present a warning and rename it (controllable)
            ...
            new_path = raw_files[0].rename(raw_path / f"{cells_filename}.json")
            return load_json(new_path, list)  # type: ignore

    # 4. If there are multiple
    if cells_filename in [f.stem for f in raw_files]:
