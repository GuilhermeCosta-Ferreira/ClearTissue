...
print(
    "Creating SINGLE PNG with 10 axial cuts (points inside prisms + density heatmap)...\n"
)
...
for filename in json_files:
    ...
    if not os.path.exists(orientation_path):
        print(
            f"❌ Orientation line missing! Run the previous script first {orientation_path}."
        )
        continue
    ...
    for i, idx in enumerate(indices):
        ...
        proj_U = np.dot(vec, U)
        proj_V = np.dot(vec, V)
        inside_square = (np.abs(proj_U) <= prism_half_width) & (
            np.abs(proj_V) <= prism_half_width
        )
        ...
        densities = np.array(
            [
                len(tree2d.query_ball_point(p, r=density_radius_2d)) - 1
    ...
    fig.suptitle(
        f"{sample_name} — 10 Axial Cuts (2000×2000 × 500 thick)\nDensity Heatmap of points inside each prism",
        fontsize=16,
        y=0.98,
    )
    plt.tight_layout()
    ...
    output_png = os.path.join(
        axial_dir, f"{sample_name}_10_AXIAL_CUTS_2000x2000_500thick_DENSITY.png"
    )
...
print(
    "Each of the 10 panels shows ONLY the points inside that exact prism, projected to 2D, with density heatmap."
)
